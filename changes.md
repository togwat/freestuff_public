...
---
**Issue**: Adding or modifying districts will not reflect the changes in the website immediately, instead a restart is required.

**Fix**: After insert, update, or delete methods in District class is called, if the query is successful, then `district_cache` will be cleared so that a new up-to-date cache can be generated.